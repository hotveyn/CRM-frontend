...
import { usePrefabUpdateFormConf } from 'components/forms/prefab/update/prefabUpdate.formconf.ts';
...
const { rules, formValues, options } = usePrefabUpdateFormConf(props.prefab);
...
async function goCreate() {
  prefabFormRef.value?.validate(async (errors) => {
    ...
    await prefabsStore.update(props.prefab, formValues);
    emits('changed');
  });
}
</script>
